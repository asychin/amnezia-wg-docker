...
# validate_client_name(), acquire_lock(), release_lock() — из common.sh
...
# get_public_ip() — используется из common.sh (режим lenient)
# Обёртка для обратной совместимости: устанавливает PUBLIC_IP
_resolve_public_ip() {
    get_public_ip lenient
...
add_client() {
    ...
    if [ -z "$client_ip" ]; then
        ...
        # БЕЗОПАСНОСТЬ: Валидация IP адреса через общую функцию
        if ! validate_ipv4 "$client_ip"; then
            exit 1
        fi
    fi

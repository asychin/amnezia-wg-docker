...
check-compose:
	@docker compose version > /dev/null 2>&1 || (echo "$(RED)Error: Docker Compose не установлен$(NC)" && exit 1)
...
.PHONY: build
build: init ## Сборка Docker образа
...
logs: check-compose ## Просмотр логов в реальном времени
	...
	@docker logs -f $(SERVICE_NAME) 2>/dev/null || echo "$(RED)Контейнер не найден$(NC)"
...
status: check-compose ## Статус сервера и соединений
	...
	@docker compose ps || echo "$(RED)Контейнер не запущен$(NC)"
	@echo ""
	@if docker compose ps | grep -q "Up"; then \
		echo "$(CYAN)🔗 Статус AmneziaWG интерфейса:$(NC)"; \
		docker exec $(SERVICE_NAME) awg show awg0 2>/dev/null || echo "$(YELLOW)Интерфейс недоступен$(NC)"; \
		echo ""; \
		echo "$(CYAN)🌐 Активные соединения:$(NC)"; \
		docker exec $(SERVICE_NAME) awg show awg0 latest-handshakes 2>/dev/null || echo "$(YELLOW)Нет активных соединений$(NC)"; \
	else \
...
.PHONY: client-add
client-add: check-compose ## Добавить клиента (name=имя [ip=IP])
	@if [ -z "$(name)" ]; then \
		echo "$(RED)❌ Укажите имя клиента: make client-add name=client1$(NC)"; \
		echo "$(YELLOW)   Или с конкретным IP: make client-add name=client1 ip=10.13.13.2$(NC)"; \
		exit 1; \
	fi
	@if [ -z "$(ip)" ]; then \
		echo "$(YELLOW)⚠️  IP не указан, будет выбран автоматически$(NC)"; \
		docker exec $(SERVICE_NAME) /app/scripts/manage-clients.sh add $(name); \
	else \
		docker exec $(SERVICE_NAME) /app/scripts/manage-clients.sh add $(name) $(ip); \
	fi
	@echo "$(GREEN)✅ Клиент $(name) добавлен$(NC)"

.PHONY: client-rm
client-rm: check-compose ## Удалить клиента (name=имя)
	@if [ -z "$(name)" ]; then \
		echo "$(RED)❌ Укажите имя клиента: make client-rm name=client1$(NC)"; \
		exit 1; \
	fi
	@docker exec $(SERVICE_NAME) /app/scripts/manage-clients.sh remove $(name)
	@echo "$(GREEN)✅ Клиент $(name) удален$(NC)"

.PHONY: client-qr
client-qr: check-compose ## Показать QR код клиента (name=имя)
	@if [ -z "$(name)" ]; then \
		echo "$(RED)❌ Укажите имя клиента: make client-qr name=client1$(NC)"; \
		exit 1; \
	fi
	@echo "$(BLUE)📱 QR код для клиента '$(name)':$(NC)"
	@docker exec $(SERVICE_NAME) qrencode -t ansiutf8 < /app/clients/$(name).conf || \
		echo "$(RED)❌ Клиент '$(name)' не найден$(NC)"

.PHONY: client-config
client-config: check-compose ## Показать конфигурацию клиента (name=имя)
	@if [ -z "$(name)" ]; then \
		echo "$(RED)❌ Укажите имя клиента: make client-config name=client1$(NC)"; \
		exit 1; \
	fi
	@docker exec $(SERVICE_NAME) cat /app/clients/$(name).conf || \
		echo "$(RED)❌ Клиент '$(name)' не найден$(NC)"

.PHONY: client-list
client-list: check-compose ## Список всех клиентов
	@docker exec $(SERVICE_NAME) /app/scripts/manage-clients.sh list

.PHONY: client-info
client-info: check-compose ## Информация о подключениях клиентов
	@echo "$(BLUE)📊 Информация о клиентах:$(NC)"
	@docker exec $(SERVICE_NAME) awg show awg0 dump 2>/dev/null || \
...
test: check-compose ## Тест соединения и конфигурации
	...
	@docker compose ps | grep $(SERVICE_NAME) | grep Up && echo "$(GREEN)✅ Контейнер запущен$(NC)" || echo "$(RED)❌ Контейнер не запущен$(NC)"
	@echo ""
	@echo "$(CYAN)2. Проверка интерфейса:$(NC)"
	@docker exec $(SERVICE_NAME) ip link show awg0 >/dev/null 2>&1 && echo "$(GREEN)✅ Интерфейс awg0 активен$(NC)" || echo "$(RED)❌ Интерфейс awg0 неактивен$(NC)"
	@echo ""
	@echo "$(CYAN)3. Проверка порта:$(NC)"
	@docker exec $(SERVICE_NAME) netstat -ulnp | grep :51820 >/dev/null 2>&1 && echo "$(GREEN)✅ Порт 51820 прослушивается$(NC)" || echo "$(RED)❌ Порт 51820 не прослушивается$(NC)"
	@echo ""
	@echo "$(CYAN)4. Проверка DNS:$(NC)"
	@docker exec $(SERVICE_NAME) nslookup google.com >/dev/null 2>&1 && echo "$(GREEN)✅ DNS работает$(NC)" || echo "$(RED)❌ Проблемы с DNS$(NC)"
...
debug: check-compose ## Отладочная информация
	...
	@docker logs --tail=20 $(SERVICE_NAME) 2>/dev/null || echo "$(YELLOW)Логи недоступны$(NC)"
	@echo ""
	@echo "$(CYAN)🌐 Сетевые интерфейсы:$(NC)"
	@docker exec $(SERVICE_NAME) ip addr show 2>/dev/null || echo "Контейнер недоступен"
	@echo ""
	@echo "$(CYAN)🔥 iptables правила:$(NC)"
	@docker exec $(SERVICE_NAME) iptables -L -n 2>/dev/null || echo "Контейнер недоступен"
...
# Проверка переменных окружения для команд клиентов
check-client-vars:
	@if [ -z "$(name)" ]; then \
